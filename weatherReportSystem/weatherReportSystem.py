...
# **Re-added missing `choose_temp` function**
...
def get_timezone(lat, lon):
    ...
    timezone_data = request(url)
    if timezone_data:
        for tz in timezone_data:
            if f"{lat:.2f}" in tz or f"{lon:.2f}" in tz:
                return tz
    return "UTC"

def display_local_time(latitude, longitude):
    timezone = get_timezone(latitude, longitude)
    local_time = datetime.datetime.now(pytz.timezone(timezone))
    st.write(f"**Local Date and Time:** {local_time.strftime('%m-%d-%Y %H:%M:%S')}")
...
def geo_lookup_city(city_name, state_code, unit):
    ...
    if city_info:
        ...
        display_local_time(lat, lon)
        fetch_weather_data(lat, lon, unit, city_display)
...
def geo_lookup_zip(zip_code, unit):
    ...
    if zip_data:
        lat, lon = zip_data['lat'], zip_data['lon']
        city_name = zip_data.get('name', 'Unknown City')
        state_code = zip_data.get('state', 'Unknown State')
        city_display = f"{city_name}, {state_code}"
        display_local_time(lat, lon)
        fetch_weather_data(lat, lon, unit, city_display)
